...
function productFromCard(card) {
    ...
    return {
        ...
        price: Number(priceElement?.dataset.usd || (priceElement?.textContent || '0').replace(/[^0-9.]/g, '')),
        image: card.querySelector('img')?.getAttribute('src') || ''
    };
}

function formatMn(value) {
    return `${(value * usdToMn).toLocaleString('en-US')} MN`;
}
...
function decorateProducts() {
    ...
    document.querySelectorAll('.product-price').forEach((price) => {
        ...
        const usd = Number((price.textContent || '0').replace(/[^0-9.]/g, ''));
        if (!usd) return;
        price.dataset.usd = usd;
        price.innerHTML = `$${usd.toLocaleString('en-US')} USD <span class="price-mn">/ ${formatMn(usd)}</span>`;
    });
}
...
function money(value) {
    return `$${value.toLocaleString('en-US')} USD / ${formatMn(value)}`;
}
...
function updateCheckoutLink() {
    ...
    const cart = readCart();
    const total = document.querySelector('#cart-total')?.textContent || '$0 USD';
