...
function productFromCard(card) {
    ...
    return {
        ...
        price: Number(priceElement?.dataset.mn || priceToMn(priceElement?.textContent)),
        image: card.querySelector('img')?.getAttribute('src') || ''
    };
}

function priceToMn(text) {
    const raw = text || '';
    const value = Number(raw.replace(/[^0-9.]/g, '')) || 0;
    const isUsd = /usd|\$/i.test(raw);
    return Math.round(isUsd ? value * usdToMn : value);
}
...
function decorateProducts() {
    ...
    document.querySelectorAll('.product-price').forEach((price) => {
        ...
        const mn = priceToMn(price.textContent);
        if (!mn) return;
        price.dataset.mn = mn;
